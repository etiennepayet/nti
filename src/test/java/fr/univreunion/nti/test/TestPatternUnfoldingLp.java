...
import fr.univreunion.nti.program.lp.patternunfolding.CorrectPatternRuleLpCollector;
...
class TestPatternUnfoldingLp {
	 ...
	 * 
	 * @param ruleTexts a list of rules, in
	 * String format
	 * @param logger a logger to display messages
	 * @param introductoryMessage an introductory message to display
	 * @throws Exception
	 */
	void unfoldLP(List<String> ruleTexts, Logger logger, String introductoryMessage) throws Exception {
		...
		Mode mode = new Mode(
				rules.getFirst().getHead().getRootSymbol(), List.of());
		Lp prog = new Lp("", rules, mode);
		...
		Collection<PatternRuleLp> correct = CorrectPatternRuleLpCollector.collectFrom(prog);
