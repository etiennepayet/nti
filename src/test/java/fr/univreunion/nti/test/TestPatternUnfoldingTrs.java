...
class TestPatternUnfoldingTrs {
	...
	void testPatternUnfold7() {
		// This is AProVE_06/factorial1.ari.

		Logger logger = Logger.getLogger("TestPatternUnfoldTrs");

		try {
			LinkedList<String> rules = new LinkedList<>();
			rules.add("fac(s(X),Y) -> fac(p(s(X)),times(s(X),Y))");
			rules.add("fac(0,X) -> X");
			rules.add("plus(0,X) -> X");
			rules.add("plus(s(X),Y) -> s(plus(p(s(X)),Y))");
			rules.add("times(0,X) -> 0");
			rules.add("times(s(X),Y) -> plus(Y,times(p(s(X)),Y))");
			rules.add("p(s(0)) -> 0");
			rules.add("p(s(s(X))) -> s(p(s(X)))");
			rules.add("factorial(X) -> fac(X,s(0))");

			unfoldTrs(rules, logger, "===== test pattern unfolding 7 =====");
		}
		catch (Exception e) {
			logger.info(e.toString());
			e.printStackTrace();
	...
	void testPatternUnfold8() {
		// This is EEG_IJCAR_12/emmes-nonloop-ex3_2.ari.

		Logger logger = Logger.getLogger("TestPatternUnfoldTrs");
		
		logger.info("===== test pattern unfolding 8 =====");
		...
		try {			
			...
			PatternRuleTrs r1 = parser.parsePatternRuleTrs(
			...
			PatternRuleTrs r2 = parser.parsePatternRuleTrs(
					"gt(X,Y){X->s(X),Y->s(Y)}{X->s(X),Y->0} -> true{}{}",
					variables);
			
			LinkedList<PatternRuleTrs> unfolders = new LinkedList<>();
			unfolders.add(r2);
						
			for (PatternRuleTrs u : r1.unfold(unfolders, 0, null))
	...
	/**
	 * Unfolds the first rule of the provided list using all
	 * the rules of the list.
	 * 
	 * @param ruleTexts a list of rules, in String format
	 * @param logger a logger to display messages
	 * @param introductoryMessage an introductory message to display
	 * @throws Exception
	 */
	void unfoldTrs(List<String> ruleTexts, Logger logger, String introductoryMessage) throws Exception {
		...
		assertNotNull(correct, "Aborting test: 'correct' is null");
		logger.info("Computed correct pattern rules:");
		for (PatternRuleTrs r : correct)
			logger.info(r.toString(variableNames));

		// We unfold the first rule of the TRS.
		PatternRuleTrs r1 =
				CorrectPatternRuleTrsProducer.buildTrivialPatternRule(rules.getFirst());
		Collection<PatternRuleTrs> patunf1 = r1.unfold(correct, 1, null);
		assertNotNull(patunf1, "Aborting test: 'patunf1' is null");
		logger.info("Computed unfolded rules -- iteration 1:");
		for (PatternRuleTrs r : patunf1)
			logger.info(r.toString());

		// Then, we unfold the produced unfolded rules.
		Collection<PatternRuleTrs> patunf2 = new LinkedList<>();
		for (PatternRuleTrs r : patunf1)
			patunf2.addAll(r.unfold(correct, 2, null));
		logger.info("Computed unfolded rules -- iteration 2:");
		for (PatternRuleTrs r : patunf2)
			logger.info(r.toString());
	}
