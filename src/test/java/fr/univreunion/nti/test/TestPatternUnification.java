...
/**
 * A class for testing unification of pattern terms.
 *
...
class TestPatternUnification {
	...
	void testPatternUnif10() {
		...
		try {
			...
			//
			PatternRuleTrs r = PatternRuleTrs.tryBuild(left, right, 0);
	...
	@Test
	@DisplayName("test pattern unification 12")
	void testPatternUnif12() {

		Logger logger = Logger.getLogger("TestPatternUnification12");
	...
	/**
	 * Applies the unification algorithm of [Payet, ICLP'25] to
	...
	void unifySequences(LinkedList<String> s1, LinkedList<String> s2,
		...
		while (it1.hasNext() && it2.hasNext()) {
			...
			p = SimplePatternTerm.tryBuild(p.getBaseTerm(), sigma, mu);
	...
	void unifyFact(String s1, String s2,
		...
		PatternRuleLp r = PatternRuleLp.tryBuildFact(q, 0);

		Collection<PatternSubstitution> mgus = p.unifyWith(r);
