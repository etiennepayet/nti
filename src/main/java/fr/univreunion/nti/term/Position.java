...
public class Position implements Iterable<Integer> {
	 ...
	 */
	private Position(List<Integer> elements) {
	...
	public Position addFirst(int i) {
		List<Integer> elementsCopy = new ArrayList<>(this.elements.size() + 1);
		elementsCopy.add(i);
		elementsCopy.addAll(this.elements);

		return new Position(elementsCopy);
	}
	...
	public Position append(Position p) {
		List<Integer> elementsCopy =
				new ArrayList<>(this.elements.size() + p.elements.size());
		elementsCopy.addAll(this.elements);
		elementsCopy.addAll(p.elements);
	...
	public Position properPrefix() {
		...
		int prefixSize = this.elements.size() - 1;
		List<Integer> elementsCopy = new ArrayList<>(prefixSize);
		elementsCopy.addAll(this.elements.subList(0, prefixSize));
