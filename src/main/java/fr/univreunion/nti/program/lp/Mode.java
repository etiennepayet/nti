...
public class Mode implements Iterable<Integer> {
	 ...
	 * 
	 * @param atom an atom
	 * @throws IllegalArgumentException if <code>atom</code> is <code>null</code>
	 */
	public Mode(Function atom) throws IllegalArgumentException
	{
		if (atom == null)
			throw new IllegalArgumentException("construction of a mode from a null term");
		
		this.predSymbol = atom.getRootSymbol();

		int n = this.predSymbol.getArity();
		for (int p = 0; p < n; p++) {
			Term argument = atom.getChild(p);
			if (argument instanceof Function) {
				FunctionSymbol f = argument.getRootSymbol();
	...
	public String toString() {
		...
		StringBuilder s = new StringBuilder(this.predSymbol.toString());
