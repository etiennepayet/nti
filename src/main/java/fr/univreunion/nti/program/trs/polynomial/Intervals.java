...
public class Intervals {
	 ...
	 */
	private final Map<PolynomialConst, Integer> minvals = new HashMap<>();

	/**
	 * The upper limits of the intervals of values.
	 */
	private final Map<PolynomialConst, Integer> maxvals = new HashMap<>();
	...
	public String toString() {
		...
		for (Entry<PolynomialConst, Integer> e : this.minvals.entrySet()) {
			...
			s1.append(e.getValue()).append(" <= ").append(c);
			
			Integer max = this.maxvals.get(c);
			if (max != null) s1.append(" <= ").append(max);
		}
		...
		for (Entry<PolynomialConst, Integer> e : this.maxvals.entrySet()) {
			...
			if (this.minvals.get(c) == null) {
				...
				s2.append(c).append(" <= ").append(e.getValue());
			}
		}
		
		if (!s1.isEmpty() && !s2.isEmpty()) s1.append(", ");
		s1.append(s2);
		s1.append("]");
