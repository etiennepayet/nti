...
public class ParentTrsLoopComp extends ParentTrs {
	 ...
	 */
	public synchronized static ParentTrsLoopComp getInstance(
			UnfoldedRuleTrsLoopComp father, RuleTrs mother,
			Position position, boolean leftOrRight) {

		if (Options.getInstance().isInVerboseMode())
			return new ParentTrsLoopComp(father, mother, position, leftOrRight);
	 ...
	 * <code>false</code> for right)
	 */
	private ParentTrsLoopComp(UnfoldedRuleTrsLoopComp father, RuleTrs mother,
			Position position, boolean leftOrRight) {
	...
	public String toString(int indentation) {
		...
		if (this.getMother() == null) {
			...
			s.append(p);
			s.append(" in the right-hand side of the first rule of ");
			s.append(l);
			...
			s.append("the subterm at position ");
			s.append(p);
			s.append(" in the left-hand side of the second rule of " + l + ".");
		}
		else if (this.getPosition().isEmpty()) {
			s.append("\n");
			s.append(spaces);
			s.append("We merge the first and the second rule of " + l + ".");
		}
		else {
			s.append("\n");
			s.append(spaces);
			...
			s.append("\n");
			s.append(spaces);
			s.append(this.getLeftOrRight() ?
					"We unfold the second rule of " + l + " backwards" :
						"We unfold the first rule of " + l + " forwards");
