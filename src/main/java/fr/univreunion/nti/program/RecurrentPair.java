...
/**
 * A recurrent pair for proving the existence of a
 * binary chain (see Sect. 5 of [Payet, JAR'24]).
 * 
...
public class RecurrentPair {
	...
	/**
	 * A nonterminating term generated by
	 * this recurrent pair.
	 */
							...
							if (c2 != null) {
								...
								if (0 <= n1) {
									...
									if (0 <= n4) {
										...
										if (t != null) {
											// We build the nonterminating term.

											// The arguments of the nonterminating term:
	...
	private synchronized static RecurrentPair buildRecPair(
		...
		// Now, we try to compute c1, c2, s, t,
		// m1, m2, n1, ..., n4.
		...
		for (int i = 0; i < n; i++) {
			...
			if (x1.isVariable() && x2.isVariable() &&
				...
				// We identify the index j of square'.
				for (int j = 0; j < n; j++)
					if (j != i) {
						...
						if (s.isGround()) {
							...
							Term u1_j = u1.getChild(j);
							Term v1_j = v1.getChild(j);
							Set<Variable> vars_u1_j = u1_j.getVariables();
							Set<Variable> vars_v1_j = v1_j.getVariables();

							if (vars_u1_j.size() == 1 && vars_v1_j.size() == 1) {
								Variable y1 = vars_u1_j.iterator().next();
								if (x1 != y1 &&
										y1 == vars_v1_j.iterator().next() &&
										occursOnlyAt(y1, u1, -1, j, n) &&
										occursOnlyAt(y1, v1, -1, j, n) &&
										occursOnlyAt(x2, v2, i, j, n)) {
									...
									// 0x25A1 is the UFT-16 encoding of
									// the white square character.
									Tuple res = groundContextFrom(u1_j, y1, new Hole("" + '\u25A1'));
									...
									// is built from u1_j, which contains y1.

									int n2 = towerOfContexts(v1_j, c2, y1);
									if (0 <= n2 && n2 < m1) {
										int n1, n3, n4, m2, delta = m1 - n2;

										if ((n1 = towerOfContexts(v1.getChild(i), c2, x1)) < 0 ||
												(0 < n1 % delta) ||
												(n4 = towerOfContexts(v2.getChild(j), c2, x2)) < 0 ||
												n4 < n2)
											continue;

										Term v2_i = v2.getChild(i);
										Term t = null, s_m2 = s; // s_m2 is c2^m2[s]

										if (0 <= (n3 = towerOfContexts(v2_i, c2, x2))) {
											// Here, t = x2 necessarily
											// and s is a subterm of u2_j. 
											if (0 < n3 % delta) continue;
											for (m2 = n2; m2 <= n4; m2++) {
												// If we don't have s = c2^m2[...]
												// then we won't have s = c2^m[...]
												// for any m >= m2.
												if ((s = towerOfContexts(s_m2, c2, m2)) == null)
													m2 = n4 + 1; // in order to exit the 'for' loop
												if ((n4 - m2) % delta == 0) break;
											}
											if (m2 > n4) continue; // Here, we could not compute m2.
											t = x2;
										}
										else if (0 <= (n3 = towerOfContexts(v2_i, c2, s))) {
											// Here, t = s necessarily
											// and s is a subterm of u2_j. 
											for (m2 = n2, n3 += n2; m2 <= n4; m2++, n3++) {
												// If we don't have s = c2^m2[...]
												// then we won't have s = c2^m[...]
												// for any m >= m2.
												if ((s = towerOfContexts(s_m2, c2, m2)) == null)
													m2 = n4 + 1; // in order to exit the 'for' loop
												if (n3 % delta == 0 && (n4 - m2) % delta == 0) break;
											}
											if (m2 > n4) continue; // Here, we could not compute m2.
											t = s;
										}
										else if (0 <= (m2 = towerOfContexts(s, c2, v2_i)) &&
												n2 <= m2 && m2 <= n4 && (n4 - m2) % delta == 0) {
											// Here, s = t = v2_i. Hence, it is useless
											// to proceed as above, i.e., to consider
											// the inner subterms of v2_i.
											n3 = 0; // consequently, n3 % delta = 0
											s = v2_i;
											t = v2_i;
										}
										else continue;

										Function nonterm = buildNT(
												u1, v1, u2, v2, f, n, s, s_m2, i, j);

										return (nonterm == null ? null :
											new RecurrentPair(u1, v1, u2, v2,
													i, j, c2, s, t,
													m1, m2, n1, n2, n3, n4,
													nonterm));
									}
	...
	/**
	 * Tries to build a nonterminating term from
	 * the provided elements. If such a term
	 * cannot be built then <code>null</code>
	 * is returned.
	 * 
	 * @param u1 the lhs of the first finite chain
	 ...
	 * @param v2 the rhs of the second finite chain
	 * of the built recurrent pair
	 * @param f the leading function symbol of c1
	 * @param n the arity of <code>f</code>
	 * @param s the ground term s of the built
	 * recurrent pair
	 * @param s_m2 the ground term c2^m2[s] of
	 * the built recurrent pair
	 * @param i the position of square in c1
	 * @param j the position of square' in c1
	 * @return a nonterminating term, or <code>null</code>
	 */
	private synchronized static Function buildNT(
			Function u1, Function v1, Function u2, Function v2,
			FunctionSymbol f, int n,
			Term s, Term s_m2,
			int i, int j) {
		...
		for (int k = 0; k < n; k++)
			if (k == i) arguments.add(s);
			else if (k == j) arguments.add(s_m2);
			else {
				Term u1_k = u1.getChild(k);
				if (!(u1_k.unifyWith(v1.getChild(k)) &&
						u1_k.unifyWith(u2.getChild(k)) &&
						u1_k.unifyWith(v2.getChild(k))))
					// If a nonterminating term cannot be built
					// then we stop because another i or another j
					// will not work. Indeed, at that point, we have
					// u1_j = c2^m1[y] and v1_j = c2^n2[y] with n2 < m1,
					// i.e., u1_j does not unify with v1_j. We have
	 ...
	 * Moreover, <code>c</code> is supposed to
	 * be <code>square</code> or to have the
	 * following form: f(t_1,...,t_n) where,
	 * for each i, t_i is square or it does not
	 * contain square.
	 * 
	 ...
	 * terms
	 */
	private synchronized static Tuple groundContextFrom(
			Term c_y, Variable y, Hole square) {

		if (c_y == y) return new Tuple(0, square);

		if (c_y instanceof Function) {
			FunctionSymbol f = c_y.getRootSymbol();
			int a = f.getArity();
			LinkedList<Term> arguments = new LinkedList<Term>();
			LinkedList<Term> holes = new LinkedList<Term>();
			Term c_y_k;
			for (int k = 0; k < a; k++)
				if ((c_y_k = c_y.get(k)).isGround())
					// c_y_k does not contain y.
					arguments.add(c_y_k);
				else {
					arguments.add(square);
					holes.add(c_y_k);
				}

			Term c = new Function(f, arguments);

			int n = -1, n_t;
			for (Term t : holes) {
				// t is a direct subterm of c_y.
				if ((n_t = towerOfContexts(t, c, y)) < 0) return null;
				if (n == -1) n = n_t;
				else if (n_t != n) return null;
			}

			// We return n + 1 because n is
			// the power of c relatively to
			// a direct subterm of c_y, i.e.,
			// c_y = c[c^n[y]].
			return new Tuple(n + 1, c);
		}
	...
	/**
	 * Checks whether the provided term <code>t</code>
	 * has the form <code>c^n[base]</code> for some
	 * term <code>base</code>. If so, then returns
	 * <code>base</code>. Otherwise, returns
	 * <code>null</code>.
	 * 
	 * In particular, <code>null</code> is returned
	 * if <code>c</code> does not contain any hole.
	 * 
	 * @param t a term to be checked
	 * @param c a context (possibly with several holes)
	 ...
	 */
	private synchronized static Term towerOfContexts(Term t, Term c, int n) {
		// We have t = c^0[t].
		if (n <= 0) return t;

		FunctionSymbol f = t.getRootSymbol();
		if (f == c.getRootSymbol()) {
			Term c_k, base_k, base = null; // base is the result to return
			int a = f.getArity();
			for (int k = 0; k < a; k++)
				if ((c_k = c.get(k)) instanceof Hole) {
					if ((base_k = towerOfContexts(t.get(k), c, n - 1)) == null)
						return null;
					if (base == null) base = base_k;
					else if (!base_k.deepEquals(base))
						return null;
	 ...
	 * @param nonterminating a nonterminating term
	...
	public String toString() {
		...
		String certificate = this.nonterminating.toString(variables, false);

		return
				"u1 -> v1 = " + this.u1.toString(variables, false) +
				" -> " + this.v1.toString(variables, false) +
				"\nu2 -> v2 = " + this.u2.toString(variables, false) +
				" -> " + this.v2.toString(variables, false) +
				"\nWe have:" +
				"\nu1 = c1[x,c2^m1[y]], v1 = c1[c2^n1[x],c2^n2[y]]" +
				"\nu2 = c1[x,c2^m2[s]], v2 = c1[c2^n3[t],c2^n4[x]]" +
				"\nfor:" +
				"\nc1 = " + this.c1.toString(variables, false) +
				"\nc2 = " + this.c2.toString(variables, false) +
				(this.s == this.t ? "\ns = t = " + this.s.toString(variables, false) :
					"\ns = " + this.s.toString(variables, false) +
					"\nt = " + this.t.toString(variables, false)) +
				"\n(m1,m2) = (" + this.m1 + "," + this.m2 + ")" +
				" and (n1,n2,n3,n4) = (" + this.n1 +
				"," + this.n2 +
				"," + this.n3 +
				"," + this.n4 + ")" +
				"\nNonterminating term = " + certificate;
	}

	/**
	 * This class is used for returning
	 * multiple values from method
	 * <code>groundContextFrom</code>.
	 * 
	 * @author <A HREF="mailto:etienne.payet@univ-reunion.fr">Etienne Payet</A>
	 */
